...
public class GraphL4A extends Graph {
	...
	private int[] d; //for DFS_numb
	private int[] f; //for DFS_numb
	private int[] subGraph; // Tab of connected componant
	...
	//method to be applied only when type=1 and weighted=1
	  ...
	  public void DFSNumb()
	    ...
	    /*-------------------------------------------------------------------Find Connected Componant/Undirected GraphList --------------------------------------*/
	    
	  //Start the recursive method
		  public void DFS_CcRecW(int s, int cpt)
		  {
			  //Mark my source as visited 
			  this.subGraph[s] = cpt;
			  WeightedNode4A ptr = new WeightedNode4A(0,this.adjlistW[s], (float)0);
			  ...
			  {
				  //Check if my vertex is not visited
				  if(this.subGraph[ptr.getVal()] == 0)
				  {
					  //Start the recursive on the target/neighbor of s
					  DFS_CcRecW(ptr.getVal(), cpt);
				  }
				  //Next target
				  ptr = ptr.getNext();
			  }
		  ...
		  //Same method as DFNumbRecW
		  public void DFS_CcRec(int s, int cpt)
		  {
			  this.subGraph[s] = cpt;
			  Node4A ptr = new Node4A(0,this.adjlist[s]);
		  ...
		  public void DFS_CC()
		    {
		    	...
		    	}else
		    	{
		    		for(int i= 0; i < this.n; i++)
		    		{
		    			...
		    			if(this.subGraph[i] == 0)
		    			{
		    				System.out.println("Lance la func rec");
		    				 DFS_CcRec(i,cpt);
		    				 System.out.println("On incremente la boucle");
		    				 cpt++;
		    			}
		    		}
		    	}
		    }
		  public void printsubGraph(int[] tab)
		  {
			int acc = 1;
			int tmp = 0;
			while (tmp <= tab.length)
			{
				for (int i = 0; i<tab.length; i++)
				{
					if (tab[i] == acc)
		  ...
		  /*-------------------------------------------------Find connected componant in a digraph --------------------------------------------------------------*/
		  public int indMaxTab(int[] tab)
		  {
			  int MaxT = 0;
			  int indMax = 0;
			  for (int i = 0; i < tab.length; i++)
		  ...
		  public void DFS_SCC()
		    {
		    	...
		    	if (this.weighted == 1)
		    	{
		    		while (this.f[indMaxTab(this.f)] != 0)
		    		{

		    			//Check if the source is visited			
		    			if(this.subGraph[indMaxTab(this.f)] == 0)
		    			{
		    				DFS_CcRecW(indMaxTab(this.f),cpt);
		    				cpt++;
		    				this.f[indMaxTab(this.f)] = 0;
		    			}
		    			else
		    			{
		    				this.f[indMaxTab(this.f)] = 0;
		    	...
		    	}else
		    	{
		    		while (this.f[indMaxTab(this.f)] != 0)
		    		{
		    			...
		    			if(this.subGraph[indMaxTab(this.f)] == 0)
		    			{
		    				this.f[indMaxTab(this.f)] = 0;
		    				System.out.println("Lance la func rec");
		    				DFS_CcRec(indMaxTab(this.f),cpt);
		    				System.out.println("On incremente la boucle");
		    				cpt++;
		    			}
		    			else
		  ...
		  /*---------------------------------------------Function to select component between digraph or undigraph -------------------------------------------*/
		  public void component(GraphL4A graph)
		  {
			  if (this.type == 0)
			  {
				  System.out.println("Undirected Graph");
				  this.DFS_CC();
				  printsubGraph(this.subGraph);
			  }
			  else
			  {
				  ...
				  graph.f = this.f;
				  printTab(this.f);
				  System.out.println("On lance DFS sur la transposée");
